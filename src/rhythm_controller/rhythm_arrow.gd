...
const HIT_WINDOW_BEATS: float = 0.3 # keep in sync with the controller (shared constant/autoload ideally)
...
func setup(angle, _direction, color, _target_beat: float, _lead_beats: float) -> void:
...
func _process(_delta: float) -> void:
	...
	position = direction * travel_distance * progress
	...
	if Conductor.current_beat > target_beat + HIT_WINDOW_BEATS:
		enabled = false
		missed.emit(direction, Conductor.current_beat - target_beat)
		queue_free()
